...
#warning "Cost-effectiveness parameters hard-coded"
  ...
  // At the epidemic start date, seed the epidemic with one high risk
  // infected person per sex aged 25 (10 years since debut).
  if (init_year == parameters.init_year) {
    y[offset(F, HIGH, 10, ANTE) + Y1NWT] = 1.0;
    y[offset(M, HIGH, 10, ANTE) + Y1NWT] = 1.0;
  }
  ...
  for (unsigned int step(0); step < years; ++step) {
    ...
    // at the epidemic start date, seed the epidemic with one high
    // risk infected person per sex aged 25 (10 years since debut)
    if (step + init_year + 1 == parameters.init_year) {
      y[offset(F, HIGH, 10, ANTE) + Y1NWT] = 1.0;
      y[offset(M, HIGH, 10, ANTE) + Y1NWT] = 1.0;
      // reset the integration step size
      drv->h = hstart;
    }
