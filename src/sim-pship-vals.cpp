...
// Date: 2015-03-06
//
// The simulator in this file was forked from simulate.cpp r1710. The
// purpose of this simulator is to measure attributes of sexual
// behavior at 2012. This must be compiled with the -DHISTORY and
// -DDEBUG flags so that full histories, including all partnership
// formation sevents, are recorded for analysis.

static struct options_t {
  options_t() : epidemic(""), analysis(""), partners(""), years(60), seed(0), art(0), prep(0) {}
  ...
  void print_help() {
    cout << "USAGE: simulate [options]\n\n"
	 ...
	 << "-p --partners=FILE   initialize partnership parameters from FILE\n"
  ...
  void parse_args(int argc, char **argv) {
    ...
    option longopts[] = {
      ...
      {"partners", required_argument, 0, 'p'},
  ...
  string partners; // partnership inputs
...
int read_partners(std::string const& filename, Parameters& params) {
  ...
  if (input.size() != 11) return -2;

  params.damp_concurrency[ MALE ][LEAST] = input[0];
  params.damp_concurrency[ MALE ][LOW  ] = input[1];
  params.damp_concurrency[ MALE ][MID  ] = input[2];
  params.damp_concurrency[ MALE ][HIGH ] = input[3];

  params.damp_concurrency[FEMALE][LEAST] = input[4];
  params.damp_concurrency[FEMALE][LOW  ] = input[5];
  params.damp_concurrency[FEMALE][MID  ] = input[6];
  params.damp_concurrency[FEMALE][HIGH ] = input[7];

  params.rate_breakup[LEAST][LEAST] = 1.0 / input[8];
  params.rate_breakup[LEAST][LOW  ] = 1.0 / input[8];
  params.rate_breakup[LEAST][MID  ] = 1.0 / input[8];
  params.rate_breakup[LEAST][HIGH ] = 1.0 / input[8];

  params.rate_breakup[LOW  ][LEAST] = 1.0 / input[8];
  params.rate_breakup[LOW  ][LOW  ] = 1.0 / input[8];
  params.rate_breakup[LOW  ][MID  ] = 1.0 / input[8];
  params.rate_breakup[LOW  ][HIGH ] = 1.0 / input[8];

  params.rate_breakup[MID  ][LEAST] = 1.0 / input[8];
  params.rate_breakup[MID  ][LOW  ] = 1.0 / input[8];
  params.rate_breakup[MID  ][MID  ] = 1.0 / input[9];
  params.rate_breakup[MID  ][HIGH ] = 1.0 / input[9];

  params.rate_breakup[HIGH ][LEAST] = 1.0 / input[ 8];
  params.rate_breakup[HIGH ][LOW  ] = 1.0 / input[ 8];
  params.rate_breakup[HIGH ][MID  ] = 1.0 / input[ 9];
  params.rate_breakup[HIGH ][HIGH ] = 1.0 / input[10];
...
int main(int argc, char **argv) {
  ...
  if (options.partners.size() == 0) {
    cout << "% Default partnership inputs used\n";
    ...
    if (read_partners(options.partners, params) < 0) {
      cerr << "Error: failed to read partnership inputs, exiting." << endl;
      return -1;
    } else {
      cout << "% Partnership inputs: " << options.partners << '\n';
    }
  }
